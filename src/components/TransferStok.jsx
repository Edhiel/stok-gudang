...
function TransferStok({ userProfile }) {
  ...
  useEffect(() => {
    ...
    const unsubDepots = onValue(
      ...
      (snapshot) => {
        ...
        const depotList = Object.keys(data).map((key) => ({
          id: key,
          name: data[key]?.info?.name || key, // fallback nama
        }));
    ...
    const unsubOutgoing = onValue(
      ...
      (err) => console.error('outgoing read error:', err)
    );
    ...
    const unsubIncoming = onValue(
      ...
      (err) => console.error('incoming read error:', err)
    );

    /* --- fail-safe loader timeout --- */
    const failSafe = setTimeout(() => {
      console.warn('Fallback: stop loading after 8 s');
      setLoading(false);
    }, 8000);
  ...
  const handleRemoveFromList = (idx) => setTransferItems(transferItems.filter((_, i) => i !== idx));
          ...
          <>
            ...
            <div className="hidden print:block p-4 text-sm">
              ...
              <table className="table w-full table-compact">
                ...
                <tbody>{transferItems.map((it, idx) => <tr key={idx}><td>{idx + 1}</td><td>{it.name}</td><td>{it.displayQty}</td></tr>)}</tbody>
              </table>
