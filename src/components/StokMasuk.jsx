...
function StokMasuk({ userProfile }) {
  ...
  useEffect(() => {
    ...
    get(assignedSuppliersRef).then((snapshot) => {
      const assignedSupplierIds = snapshot.exists() ? Object.keys(snapshot.val()) : [];
      onValue(masterItemsRef, (masterSnapshot) => {
        const masterData = masterSnapshot.val() || {};
        const allMasterItems = masterData ? Object.keys(masterData).map(key => ({ id: key, ...masterData[key] })) : [];
        const relevantItems = allMasterItems.filter(item => assignedSupplierIds.includes(item.supplierId));
        setAvailableItems(relevantItems);
      });
    });
  }, [userProfile.depotId]);
  ...
  const handleSelectItem = async (item) => {
    ...
    if (snapshot.exists()) {
      setItemStock({ total: snapshot.val().totalStockInPcs || 0, damaged: snapshot.val().damagedStockInPcs || 0 });
    } else {
      setItemStock({ total: 0, damaged: 0 });
    }
  };
    ...
    try {
      for (const transItem of transactionItems) {
        ...
        await runTransaction(stockRef, (currentStock) => {
          if (!currentStock) {
            return { totalStockInPcs: transItem.quantityInPcs, damagedStockInPcs: 0 };
          }
            ...
            <div className="p-4 border rounded-lg bg-base-200">
                ...
                {filteredItems.length > 0 && !selectedItem && (
                  <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-full max-h-60 overflow-y-auto">
                    {filteredItems.slice(0, 5).map(item => (
                      <li key={item.id}><a onClick={() => handleSelectItem(item)}>{item.name} ({item.supplierName})</a></li>
                    ))}
                  </ul>
                )}
              </div>
              {selectedItem && (
                <div className="mt-4">
                    ...
                    <p className="text-sm">Stok Saat Ini: {itemStock.total} Pcs</p>
