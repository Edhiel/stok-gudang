...
function Dashboard({ user, setPage }) {
  ...
  useEffect(() => {
    if (!user || !user.depotId) {
      setLoading(false);
      toast.error('Profil pengguna atau ID depo tidak ditemukan.');
      return;
    }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const unsubscribe = onSnapshot(
          query(collection(firestoreDb, `depots/${user.depotId}/transactions`)),
          (snapshot) => {
            ...
            transactions.forEach((tx) => {
              if (tx.timestamp && tx.timestamp.toDate) {
                ...
                if (txDate >= sevenDaysAgo) {
                  ...
                  } else if (tx.type === 'Stok Keluar') {
                    dailyData[dateStr].keluar += tx.items.reduce((sum, item) => sum + item.qtyInPcs, 0);
                  }
                }
              }
            });
        ...
        const [masterSnapshot, supplierSnapshot, stockSnapshot, ordersSnapshot] = await Promise.all([
        ...
        const suppliers = supplierSnapshot.docs.reduce((acc, doc) => ({ ...acc, [doc.id]: doc.data() }), {});
        const stockData = stockSnapshot.docs.reduce((acc, doc) => ({ ...acc, [doc.id]: doc.data() }), {});
        const pendingOrders = ordersSnapshot.docs.length;
    ...
    return () => {
      // Cleanup onSnapshot subscriptions if needed
    };
  }, [user]);
