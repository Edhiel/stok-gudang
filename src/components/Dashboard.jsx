...
const SubMenuCard = ({ name, icon, onClick, category, colorSchemes }) => {
...
const colorSchemes = {
  orderan: '#0891B2', operasional: '#2563EB', master: '#16A34A', admin: '#9333EA', laporan: '#D97706', pengiriman: '#F59E0B',
};
...
function Dashboard({ user, setPage }) {
  ...
  useEffect(() => {
    ...
    const unsubTransactions = onSnapshot(transactionsQuery, (snapshot) => {
        ...
        setChartData({
          ...
          datasets: [
            { label: 'Stok Masuk (Pcs)', data: Object.values(dailyData).map((d) => d.masuk), backgroundColor: 'rgba(75, 192, 192, 0.5)' },
            { label: 'Stok Keluar (Pcs)', data: Object.values(dailyData).map((d) => d.keluar), backgroundColor: 'rgba(255, 99, 132, 0.5)' }
          ],
        });
    });
  ...
  const mainMenu = useMemo(() => {
      ...
      return [
        ...
        { title: 'Orderan & Faktur', icon: <Ikon.ProsesOrder />, items: menuOrderan, show: !isDriverOrHelper && menuOrderan.length > 0 },
        { title: 'Aktivitas Gudang', icon: <Ikon.StokMasuk />, items: menuGudang, show: !isDriverOrHelper && canDoGudangTransaction },
        { title: 'Master Data', icon: <Ikon.MasterBarang />, items: menuMaster, show: !isDriverOrHelper && canAccessMasterData },
        { title: 'Laporan & Analitik', icon: <Ikon.Laporan />, items: menuLaporan, show: !isDriverOrHelper && canViewLaporan },
        { title: 'Konfigurasi & Admin', icon: <Ikon.User />, items: menuAdmin, show: !isDriverOrHelper && isSuperAdmin },
  ...
  const isDriverOrHelper = user?.role === 'Sopir' || user?.role === 'Helper Depo';
  const isSales = user?.role === 'Sales Depo';
