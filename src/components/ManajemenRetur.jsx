...
const TabReturBaik = ({ userProfile, syncOfflineReturns, setActiveTab }) => {
      ...
      <div className="p-4 space-y-4">
        <div className="p-4 border rounded-lg bg-base-200 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="form-control">
              <label className="label"><span className="label-text font-bold">No. Faktur Asal</span></label>
              <input type="text" value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} placeholder="No. Faktur penjualan" className="input input-bordered" />
            </div>
            <div className="form-control">
              <label className="label"><span className="label-text font-bold">Nama Toko Asal</span></label>
              <input type="text" value={storeName} onChange={(e) => setStoreName(e.target.value)} placeholder="Toko yang mengembalikan barang" className="input input-bordered" />
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border rounded-lg bg-base-200 space-y-4">
          ...
          {searchTerm.length > 0 && !selectedItem && (
            <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-full max-h-60 overflow-y-auto">
              {items.filter(item => item.name.toLowerCase().includes(searchTerm.toLowerCase())).map(item => (<li key={item.id}><a onClick={() => handleSelectItem(item)}>{item.name}</a></li>))}
            </ul>
          )}
          {selectedItem && (
            <div className="mt-2">
              <p className="font-bold">Barang Terpilih: <span className="text-secondary">{selectedItem.name}</span></p>
              <div className="divider">Masukkan Jumlah Retur</div>
        ...
        <div className="overflow-x-auto">
          <table className="table w-full">
            <thead><tr><th>Nama Barang</th><th>Jumlah Retur</th><th>Aksi</th></tr></thead>
            <tbody>{transactionItems.map((item, index) => (<tr key={index}><td>{item.name}</td><td>{item.displayQty}</td><td><button onClick={() => handleRemoveFromList(index)} className="btn btn-xs btn-error">Hapus</button></td></tr>))}</tbody>
          </table>
        </div>
...
const TabReturRusak = ({ userProfile, syncOfflineReturns, setActiveTab }) => {
  const [items, setItems] = useState([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedItem, setSelectedItem] = useState(null);
  const [dosQty, setDosQty] = useState(0);
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="p-4 bg-base-200 rounded-lg mb-4 grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="form-control"><label className="label-text">Dari Tanggal</label><input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="input input-bordered" /></div>
        <div className="form-control"><label className="label-text">Sampai Tanggal</label><input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="input input-bordered" /></div>
        <div className="form-control"><label className="label-text">Atau Pilih Bulan</label><select onChange={handleMonthSelect} className="select select-bordered"><option value="">Pilih Bulan...</option>{monthOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select></div>
        <div className="form-control"><label className="label-text">Filter Tipe Transaksi</label><select value={filterType} onChange={e => setFilterType(e.target.value)} className="select select-bordered"><option value="">Semua Tipe</option>{transactionTypes.map(type => <option key={type} value={type}>{type}</option>)}</select></div>
      </div>
      <div className="overflow-x-auto">
        <table className="table table-zebra w-full">
          <thead><tr><th>Tanggal</th><th>Tipe</th><th>Detail</th><th>Oleh</th></tr></thead>
          <tbody>
            {loading ? (<tr><td colSpan="4" className="text-center"><span className="loading loading-dots"></span></td></tr>)
            ...
            : (filteredTransactions.map(trans => (
                <tr key={trans.id}>
                    ...
                    <td><div className="text-xs">{trans.items.map(item => `${item.name} (${item.displayQty})`).join(', ')}{trans.fromStore && <span className="block">Dari: {trans.fromStore}</span>}{trans.documentNumber && <span className="block">No. Dok: {trans.documentNumber}</span>}</div></td>
                    <td>{trans.user}</td>
                </tr>
            )))}
          </tbody>
  ...
  return (
    <div className="p-8">
      ...
      <div className="bg-white p-6 rounded-b-lg rounded-tr-lg shadow-lg min-h-96">
        {activeTab === 'returBaik' && <TabReturBaik userProfile={userProfile} syncOfflineReturns={syncOfflineReturns} setActiveTab={setActiveTab}/>}
        {activeTab === 'returRusak' && <TabReturRusak userProfile={userProfile} syncOfflineReturns={syncOfflineReturns} setActiveTab={setActiveTab}/>}
