...
// Impor fungsi-fungsi Firebase yang kita butuhkan
import { ref, onValue, update, push, serverTimestamp, child } from 'firebase/database';
import { db } from '../firebaseConfig';

// Fungsi bantuan untuk format Dos.Pack.Pcs
...
function StockOpname({ userProfile, setPage }) {
  ...
  const handleSaveChanges = async () => {
    if (reportData.length === 0) {
      alert("Tidak ada selisih stok untuk disimpan.");
      return;
    }
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div>
          ...
          <button 
            ...
            disabled={viewMode === 'input' || reportData.length === 0}
          >
            Simpan Penyesuaian
          </button>
        </div>
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <div className="flex gap-2">
                        <input type="number" placeholder="Dos" className="input input-bordered input-sm w-1/3" onChange={(e) => handleCountChange(item.id, 'dos', e.target.value)} defaultValue={physicalCounts[item.id]?.dos || ''} />
